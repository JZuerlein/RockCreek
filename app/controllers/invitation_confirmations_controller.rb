class InvitationConfirmationsController < ApplicationController
  ...
  def update
    ...
    @user = User.new(:password => params[:invite_user][:password], :password_confirmation => params[:invite_user][:password_confirmation], :email => params[:invite_user][:email], :first_name => params[:invite_user][:first_name], :last_name => params[:invite_user][:last_name], :tenant_id => @invite_user.tenant_id, :confirmed => TRUE)
